...
public class ProceduralAudioController : MonoBehaviour {
    ...
    public ElephantControl Elephan;
	...
	void OnAudioFilterRead(float[] data, int channels){
		...
		*/
		...
		for (int i = 0; i < dataLen; i++)	{ // go through data chunk
            ...
            double currentFreq = Elephan.pos_x * mainFrequency;
            if (useFrequencyModulation) {
				double freqOffset = (frequencyModulationOscillatorIntensity * mainFrequency * 0.75) / 100.0;
				currentFreq += mapValueD (frequencyModulationOscillator.calculateSignalValue (preciseDspTime, Elephan.pos_z * frequencyModulationOscillatorFrequency), -1.0, 1.0, -freqOffset, freqOffset);
